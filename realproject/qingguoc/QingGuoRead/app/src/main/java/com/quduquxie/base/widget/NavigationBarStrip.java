...
public class NavigationBarStrip extends View implements ViewPager.OnPageChangeListener {
    ...
    public NavigationBarStrip(final Context context, final AttributeSet attrs, final int defStyleAttr) {
        ...
        if (attrs != null) {
            ...
            int navigation_width = typedArray.getResourceId(R.styleable.NavigationBarStrip_navigationWidth, R.dimen.width_500);
            navigationWidth = resources.getDimensionPixelOffset(navigation_width);

            int navigation_height = typedArray.getResourceId(R.styleable.NavigationBarStrip_navigationHeight, R.dimen.height_98);
            navigationHeight = resources.getDimensionPixelOffset(navigation_height);

            int indicator_margin_bottom = typedArray.getResourceId(R.styleable.NavigationBarStrip_indicatorMarginBottom, R.dimen.height_0);
            indicatorMarginBottom = resources.getDimensionPixelOffset(indicator_margin_bottom);
            ...
            int indicator_width = typedArray.getResourceId(R.styleable.NavigationBarStrip_indicatorWidth, R.dimen.width_4);
            indicatorWidth = resources.getDimension(indicator_width);

            int indicator_padding = typedArray.getResourceId(R.styleable.NavigationBarStrip_indicatorPadding, R.dimen.width_10);
            ...
            int indicator_radius = typedArray.getResourceId(R.styleable.NavigationBarStrip_indicatorRadius, R.dimen.radius_4);
            ...
            navigationWidth = resources.getDimensionPixelOffset(R.dimen.width_500);
            navigationHeight = resources.getDimensionPixelOffset(R.dimen.height_88);

            indicatorMarginBottom = resources.getDimensionPixelOffset(R.dimen.height_0);

            indicatorColor = resources.getColor(R.color.theme_primary);
            indicatorTextSize = resources.getDimension(R.dimen.text_size_30);
            indicatorWidth = resources.getDimension(R.dimen.width_4);
            indicatorPadding = resources.getDimension(R.dimen.width_10);
            ...
            indicatorRadius = resources.getDimension(R.dimen.radius_4);
    ...
    public void setViewPager(final ViewPager viewPager) {
        ...
        if (navigationViewPager != null) {
            navigationViewPager.addOnPageChangeListener(null);
        }
    ...
    private void updateIndicatorPosition(final float fraction) {
        ...
        stripLeft = startStripX + (resizeInterpolator.getResizeInterpolation(fraction, resizeInState) * (endStripX - startStripX));

        stripRight = (startStripX + (stripType == StripType.LINE ? tabSize : indicatorWidth)) + (resizeInterpolator.getResizeInterpolation(fraction, !resizeInState) * (endStripX - startStripX));
    ...
    protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
        ...
        bounds.set(0.0F, 0.0F, navigationWidth, navigationHeight);

        if (titles.size() == 0 || navigationWidth == 0 || navigationHeight == 0) {
            return;
        }

        tabSize = navigationWidth / (float) titles.size();
        ...
        if (isInEditMode() || !viewPagerModeState) {
            ...
            startStripX = (mIndex * tabSize) + (stripType == StripType.POINT ? tabSize * 0.5F : 0.0F);
    ...
    protected void onDraw(final Canvas canvas) {
        stripBounds.set(stripLeft - (stripType == StripType.POINT ? indicatorWidth * 0.5F : 0.0F) + indicatorPadding,
                stripGravity == StripGravity.BOTTOM ? bounds.height() - indicatorWidth : 0.0F,
                stripRight - (stripType == StripType.POINT ? indicatorWidth * 0.5F : 0.0F) - indicatorPadding,
                stripGravity == StripGravity.BOTTOM ? bounds.height() - indicatorMarginBottom : indicatorWidth);
        ...
        for (int i = 0; i < titles.size(); i++) {
            ...
            final float topTitleOffset = (bounds.height() - indicatorWidth) * 0.5F + titleBounds.height() * 0.5F - titleBounds.bottom;

            final float interpolation = resizeInterpolator.getResizeInterpolation(fraction, true);
            final float lastInterpolation = resizeInterpolator.getResizeInterpolation(fraction, false);
            ...
            canvas.drawText(title, leftTitleOffset, topTitleOffset + (stripGravity == StripGravity.TOP ? indicatorWidth : 0.0F), titlePaint);
        }
    }
    ...
    public void onPageScrolled(int position, float positionOffset, final int positionOffsetPixels) {
        ...
        if (!setIndexFromTabBar) {
            ...
            startStripX = (position * tabSize) + (stripType == StripType.POINT ? tabSize * 0.5F : 0.0F);
