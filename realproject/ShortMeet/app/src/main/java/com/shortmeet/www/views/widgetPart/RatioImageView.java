...
//                R.styleable.RatioImageView_width_to_height_ratio, mWidthRatio);
        boolean baseWidth = a.getBoolean(R.styleable.RatioImageView_base_width, false);
        boolean baseHeight = a.getBoolean(R.styleable.RatioImageView_base_height, false);
        String hwRatio = a.getString(R.styleable.RatioImageView_height_width_ratio);
        if (!TextUtils.isEmpty(hwRatio)) {
            String heightWidthRatio = hwRatio.trim();
            if (TextUtils.isEmpty(heightWidthRatio)) {
                heightWidthRatio = "1:1";
            }
            String[] split = heightWidthRatio.split(":");
            if (baseWidth) {
                mHeightRatio = Integer.parseInt(split[0]) * 1.f / Integer.parseInt(split[1]);
            }

            if (baseHeight) {
                mWidthRatio = Integer.parseInt(split[1]) * 1.f / Integer.parseInt(split[0]);
            }
        }
        a.recycle();
    }
