...
//        adapter.addDraftItem();
        adapter.setData(storage.getMedias());   //adaptergallery 4919
        storage.setOnMediaDataUpdateListener(new MediaStorage.OnMediaDataUpdate() {  // mediastorge@4917
            @Override
            public void onDataUpdate(List<MediaInfo> list) {
                    int count = adapter.getItemCount();  //5
                    int size = list.size();
