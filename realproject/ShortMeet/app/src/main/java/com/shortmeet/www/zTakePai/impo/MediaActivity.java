...
package com.shortmeet.www.zTakePai.impo;
...
import com.shortmeet.www.zTakePai.impo.media.GalleryDirChooser;
import com.shortmeet.www.zTakePai.impo.media.GalleryMediaChooser;
import com.shortmeet.www.zTakePai.impo.media.MediaDir;
import com.shortmeet.www.zTakePai.impo.media.MediaInfo;
import com.shortmeet.www.zTakePai.impo.media.MediaStorage;
import com.shortmeet.www.zTakePai.impo.media.SelectedMediaAdapter;
import com.shortmeet.www.zTakePai.impo.media.SelectedMediaViewHolder;
import com.shortmeet.www.zTakePai.impo.media.ThumbnailGenerator;
...
public class MediaActivity extends Activity implements View.OnClickListener {
   ...
   // private Button mBtnNextStep;
    ...
    private TextView shortmeet_crop_net_step;
    // Fly 注：  此mediaActivity是导入编辑功能
    @Override
    ...
    private void getData() {
     //  把启动页面设置 分辨率等输入框功能 的类去掉了  写成了固定 即那个类去掉了，所以取不到值，后面当作固定值来使用
        mRatio = getIntent().getIntExtra(CropKey.VIDEO_RATIO, CropKey.RATIO_MODE_3_4);
        System.out.println("导入编辑编辑功能所使用的分辨率=====================》"+mRatio);
            ...
            public void onComplete(List<MediaInfo> resultVideos) {
                ...
                for (int i = 0; i < resultVideos.size(); i++) {
                    ...
                    if (i == 0) {
                        ...
                        if (mediaInfo.mimeType.startsWith("video")) {
                            mImport.addVideo(mediaInfo.filePath, 600, AliyunDisplayMode.DEFAULT);
                        } else if (mediaInfo.mimeType.startsWith("image")) {
            ...
            public void onCancelComplete() {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        shortmeet_crop_net_step.setEnabled(true);
                    }
                });
            }
        });
        shortmeet_crop_net_step= (TextView) findViewById(R.id.shortmeet_crop_net_step);
            ...
            public void onCurrentMediaInfoChanged(MediaInfo info) {
             /*   MediaInfo infoCopy = new MediaInfo();
                ...
                if (info.mimeType.startsWith("image")) {
                    infoCopy.duration = 5000;//图片的时长设置为5s
                } else {
...
//                mImport.addVideo(infoCopy.filePath, 3000, AliyunDisplayMode.DEFAULT);    //导入器中添加视频
                ...
                if (info.mimeType.startsWith("video")) {
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            ToastUtil.showToast(MediaActivity.this,getString(R.string.shortmeet_crop_select_tips));
                        }
                    });
...
//                mImport.removeVideo(info.filePath); //从导入器中移除视频
            ...
            switch (requestCode) {
                case REQUEST_CODE_VIDEO_CROP:
                 ...
                 /*   if (!TextUtils.isEmpty(path) && duration > 0 && mCurrMediaInfo != null) {
                    ...
                    if (!TextUtils.isEmpty(path) && duration > 0 && mCurrMediaInfo != null){
                        MediaInfo infoCopy = new MediaInfo();
                        infoCopy.addTime = mCurrMediaInfo.addTime;
                        infoCopy.duration = (int) duration;
                        infoCopy.filePath = path;
                        infoCopy.id = mCurrMediaInfo.id;
                        infoCopy.isSquare = mCurrMediaInfo.isSquare;
                        infoCopy.mimeType = mCurrMediaInfo.mimeType;
                        infoCopy.thumbnailPath = mCurrMediaInfo.thumbnailPath;
                        infoCopy.title = mCurrMediaInfo.title;
                        infoCopy.type = mCurrMediaInfo.type;
                        mSelectedVideoAdapter.addMedia(infoCopy);
                        //                mImport.addVideo(infoCopy.filePath, 3000, AliyunDisplayMode.DEFAULT);    //导入器中添加视频
                        mTransCoder.addMedia(infoCopy);
                    }
    ...
    public void onClick(View v) {
        if (v == back) {
            finish();
        } else if (v.getId() == R.id.shortmeet_crop_net_step) {//点击下一步
            ...
            if (videoCount > 0) {
                ...
                progressDialog.setOnCancelListener(new DialogInterface.OnCancelListener() {
                    ...
                    public void onCancel(DialogInterface dialog) {
                        shortmeet_crop_net_step.setEnabled(false);//为了防止未取消成功的情况下就开始下一次转码，这里在取消转码成功前会禁用下一步按钮
                        mTransCoder.cancel();
                    }
                });
                mTransCoder.init(this);
