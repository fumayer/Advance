...
qq.FileUploaderBasic = function(o){
    this._options = {
        ...
        // messages                
        messages: {
            typeError: "{file} 具有无效的扩展。只有{extensions} 允许扩展。",
            sizeError: "{file} 文件太大，最大文件大小是{sizeLimit}。",
            minSizeError: "{file} 太小，最小的文件大小是 {minSizeLimit}。",
            emptyError: "{file}是空的，请选择其他文件。",
...
qq.extend(qq.FileUploader.prototype, {
    ...
    _onProgress: function(id, fileName, loaded, total){
        ...
        var text; 
        if (loaded != total){
									   if($("#task_"+id).find(".s").last().html().length<=0){
														$("#task_"+id).find(".s").last().html(this._formatSize(total));
												}					
            ...
            text = this._formatSize(total);
								...
								//window. timer=setInterval(function(){if((window.progressCount+1)>99){clearInterval(window.timer);$("tbody").find(".uploading .e").html(99+"%");}else{$("tbody").find(".uploading .e").html(window.progressCount+"%");window.progressCount++;}},20);
        $("#task_"+id).find(".e").first().html("上传中...");
    ...
    _onComplete: function(id, fileName, result){
        ...
        if (result.success == 1){
            ...
            this._listElement.removeChild(item);
											//window.progressCount=0;
											$("#task_"+id).find(".e").first().html("上传完成");
											//$("tbody").find(".uploading").remove();
    ...
    _addToList: function(id, fileName){
        ...
        this._listElement.appendChild(item);
								$("tbody").last().append('<tr id="task_'+id+'" class="c uploading"><td class="i"></a></td><td class="n"> <a href="#">'+fileName+'</a></td><td class="m"></td><td class="s"></td><td class="e"></td><td class="e"></td></tr>');
    },
