...
public class GoldListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (position>0){//position是title,所以position=1开始，对应list是0
            bean=mList.get(position-1);
        }

        if (holder instanceof ContentViewHolder){
            if (bean.getScreenshot()!=null && bean.getScreenshot().getUrl()!=null){
                GlidUtils.load(mContext,bean.getScreenshot().getUrl(),((ContentViewHolder)holder).ivImag);
            }else{
                ((ContentViewHolder) holder).ivImag.setImageResource(R.mipmap.ic_launcher);
            }
            ...
            if (bean.getScreenshot()!=null && bean.getScreenshot().getUrl()!=null){
                GlidUtils.load(mContext,bean.getScreenshot().getUrl(),((HotViewHolder) holder).ivImag);
            }else{
                ((HotViewHolder) holder).ivImag.setImageResource(R.mipmap.ic_launcher);
            }
